...
const Playboard = () => {
    ...
    useEffect(() => {
        const fetchBoardData = async () => {
            try {
                const boardResponse = await axios.get(`${config.API_ENDPOINT}/boards/${id}`);
    ...
    const questionsForBoardMap = existingBoardQuestions.length === 0 ? (
        <p>No items here</p>
    ) : (
        existingBoardQuestions.map((question, key) => {
            const linkUrlOutput = `/playquestion/${question.question_category}/${question.id}/${question.board_id}/${question.question_points}`;
            return (
                <div className={`divTableCell ${((key + 1) % 6 === 0) ? 'lastCell' : ''}`} key={key}>
